...
//		unMarshalXMLData(path);
	...
	private void unMarshalXMLData(BufferedReader data) throws JAXBException
	{
		...
		m_MagitRepository = (MagitRepository) unmarshaller.unmarshal(data);
	}
	...
	public Utilz.TwoParametersResult checkValidationXML(BufferedReader data, String magitRepositoryPath) throws JAXBException
	{
		unMarshalXMLData(data);
		this.magitRepositoryPath=magitRepositoryPath+"\\"+getMagitRepository().getName();;
		Utilz.TwoParametersResult isRepositoryExist=isRepositoryDoesNotExist();
		if(!isRepositoryExist.m_IsValid)
		{
			return isRepositoryExist;
		}
	...
	private Utilz.TwoParametersResult isRepositoryDoesNotExist()
	{
		File file= new File(magitRepositoryPath);
		return new Utilz.TwoParametersResult(!file.exists(),"Repository by that name already exists");
	}
	...
	public Repository fetchXMLRepository() throws IOException
	{
		...
		repository = new Repository(head, magitRepositoryPath, m_MagitRepository.getName());
	...
	private void createRemote(Repository repository) throws IOException
	{
		...
		String content=remotePath+","+"Administrator";
		if(!remotePath.equals(""))
		{
			repository.createMagitFile("RemotePath", content, "branch");
		}
	}
	...
	private String getCommitChain(MagitSingleCommit magitCommit, Repository repository) throws IOException
	{
		...
		else
		{
			...
			for (PrecedingCommits.PrecedingCommit precedeCommit : magitCommit.getPrecedingCommits().getPrecedingCommit())
			{
				...
				{
					previousCommitKeys.add(getCommitChain(previousCommit, repository));
				}
			}
